...
const UsersList = ({ users }) => {
    ...
    return (
        <ul><h1 style={{marginTop:"3.3rem"}}>Users: </h1>
               {users.map((user) => {
                    return (
                        <li key={user._id} style={{ padding: "8px 16px", borderBottom: "1px solid #ddd",
                         listStyleType: "none", borderBottom: '1px solid blue' }}>
                            <div>
                                <h1>{user.name}</h1>
                                <p style={{ textDecoration: "none", color: "grey" }}>{user.email}</p>

                                <div div style={{ border: user.enabled === "0" ? '2px solid grey' : 'none'}}>
                                    <button style={{ backgroundColor: "grey", marginTop:"1rem" }} 
